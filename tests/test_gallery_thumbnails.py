...
class GalleryThumbnailTests(TestCase):
    ...
    def test_concurrent_first_requests_generate_one_atomic_thumbnail(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            thumbnail = upload_dir / "_thumbs" / "Graduation" / "photo.webp.v5.webp"
    ...
    def test_unfocused_gallery_uses_generated_thumbnails(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                images[0].startswith("/uploads/_thumbs/Graduation/photo.webp.v5.webp?v=5-")
            )
            self.assertEqual(full_images, ["/uploads/Graduation/photo.webp"])
            self.assertTrue(thumbnail.exists())

            with Image.open(thumbnail) as img:
                self.assertLessEqual(max(img.size), 960)
    ...
    def test_same_stem_different_extensions_get_distinct_thumbnails(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                image_urls[0].startswith("/uploads/_thumbs/Graduation/photo.jpg.v5.webp?v=5-")
            )
            self.assertTrue(
                image_urls[1].startswith("/uploads/_thumbs/Graduation/photo.png.v5.webp?v=5-")
            )
            thumbnail_paths = [
                upload_dir / "_thumbs" / "Graduation" / "photo.jpg.v5.webp",
                upload_dir / "_thumbs" / "Graduation" / "photo.png.v5.webp",
            ]
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                album["images"][0].startswith(
                    "/uploads/_thumbs/Trip%20%3F%23/photo%20%3F%23%2001.webp.v5.webp?v=5-"
                )
            )
    ...
    def test_changed_source_is_not_published_under_stale_cache_key(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            expected_path = upload_dir / "_thumbs" / "Graduation" / "photo.webp.v5.webp"
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                warm_payload["albums"][0]["images"][0].startswith(
                    "/uploads/_thumbs/Graduation/photo.webp.v5.webp?v=5-"
                )
            )
    ...
    def test_animated_gif_keeps_original_in_focused_and_album_views(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse(
                (upload_dir / "_thumbs" / "Graduation" / "animation.gif.v5.webp").exists()
            )
