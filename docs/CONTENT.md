...
- `public`：所有访客可见，也会进入搜索和 News。
- `private`：只有已登录上传管理员可见。
- `hidden`：不保存到 visibility map，不在 Gallery 展示。
...
常用文件放在 `uploads/`，模板当前使用的名字可在 `app/templates/pages/resume.html` 查看。文件名包含空格或中文时 URL 会编码；文件管理 API 使用相同安全路径规则。

替换现有文件后，uploads 缓存最多保留 1 小时。需要立即切换的高关注资源应让模板通过 `upload_url()` 输出版本 token，或使用新文件名。
...
`.gitignore` 默认排除大部分个性化 content、所有生产 uploads、Gallery 配置、`.env` 和 sessions。仓库保存应用与必要示例/既有受跟踪内容，生产数据必须独立备份。
