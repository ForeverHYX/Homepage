...
flowchart LR
    ...
    Nginx -->|/uploads 可变文件| Uploads[(/root/newhomepage/uploads)]
    Nginx -->|HTML 与 /api| Gunicorn[Gunicorn master]
    Gunicorn --> Worker[1 个 Uvicorn worker]
    Worker --> App[FastAPI]
    App --> Content[(content / JSON / sessions)]
    Systemd[systemd] --> Gunicorn
```

Nginx 直接服务静态和上传文件，因此这些请求不消耗 Python worker。动态页面由一个预加载 worker 服务，以减少重复内存并保持文件会话模型一致。
...
备份只放在 0700 目录；其中包含密码哈希、会话和私人相册，不应上传到公共对象存储。
...
指纹静态资源应有 `max-age=31536000, immutable` 和 `Content-Encoding: gzip`（文本资源）；无指纹 `/static/` 应为 300 秒；uploads 应为 3600 秒且不是 immutable。
...
如果需要恢复内容，先停止写入管理后台，然后从备份单独恢复 `content/`、`uploads/`、`gallery_config.json`；不要默认恢复旧 `.sessions.json`，让现有登录失效更安全。
